...
import {
    ...
    DollarSign, MapPin, Save, Loader2, Image as ImageIcon,
...
export default function Edit({ employee, shifts, auth }) {
    ...
    return (
        <FigmaLayout user={auth.user}>
            <Head title={`Update Member: ${employee.first_name}`} />
            ...
            <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem', paddingBottom: '5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <div>
                            <h1 style={{ fontSize: '1.875rem', fontWeight: 900, color: '#111827', margin: 0, letterSpacing: '-0.02em' }}>Update Member</h1>
                            <p style={{ fontSize: '0.95rem', color: '#6b7280', margin: '4px 0 0', fontWeight: 500 }}>Modifying details for {employee.first_name} {employee.last_name}.</p>
                        </div>
                    </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }} className="form-grid">
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        
                        <SectionCard title="Basic Details" subtitle="Full name and status" icon={User}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                                <div>
                                    <label style={labelStyle}>Employee ID</label>
                                    <input type="text" value={data.employee_id} onChange={e => setData('employee_id', e.target.value)} style={fieldStyle} required />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <Field icon={User} error={errors.first_name}>
                                        <input type="text" value={data.first_name} onChange={e => setData('first_name', e.target.value)} style={fieldStyle} required />
                                    </Field>
                                </div>
                                <div>
                                    <label style={labelStyle}>Last Name</label>
                                    <Field icon={User} error={errors.last_name}>
                                        <input type="text" value={data.last_name} onChange={e => setData('last_name', e.target.value)} style={fieldStyle} required />
                                    </Field>
                                </div>
                            </div>
                        </SectionCard>

                        <SectionCard title="Job Details" subtitle="Role and work shift" icon={Briefcase}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                                <div>
                                    <label style={labelStyle}>Department</label>
                                    <Field icon={Building2} error={errors.department}>
                                        <input type="text" value={data.department} onChange={e => setData('department', e.target.value)} style={fieldStyle} required />
                                    </Field>
                                </div>
                                <div>
                                    <label style={labelStyle}>Job Title</label>
                                    <Field icon={Briefcase} error={errors.designation}>
                                        <input type="text" value={data.designation} onChange={e => setData('designation', e.target.value)} style={fieldStyle} required />
                                    </Field>
                                </div>
                                <div>
                                    <label style={labelStyle}>Monthly Salary (৳)</label>
                                    <Field icon={DollarSign} error={errors.salary}>
                                        <input type="number" value={data.salary} onChange={e => setData('salary', e.target.value)} style={fieldStyle} />
                                    </Field>
                                </div>
                                <div>
                        ...
                        <SectionCard title="Contact Info" subtitle="Physical and digital reach" icon={Phone}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                                    <div>
                                        <label style={labelStyle}>Email Address</label>
                                        <Field icon={Mail} error={errors.email}>
                                            <input type="email" value={data.email} onChange={e => setData('email', e.target.value)} style={fieldStyle} />
                                        </Field>
                                    </div>
                                    <div>
                                        <label style={labelStyle}>Phone Number</label>
                                        <Field icon={Phone} error={errors.phone}>
                                            <input type="text" value={data.phone} onChange={e => setData('phone', e.target.value)} style={fieldStyle} required />
                                        </Field>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                                        <input type="text" value={data.emergency_contact_name} onChange={e => setData('emergency_contact_name', e.target.value)} placeholder="Name" style={{ ...fieldStyle, paddingLeft: '1rem' }} />
                                        <input type="text" value={data.emergency_contact_phone} onChange={e => setData('emergency_contact_phone', e.target.value)} placeholder="Phone" style={{ ...fieldStyle, paddingLeft: '1rem' }} />
                                    </div>
                                </div>
                                <div>
                                    <label style={labelStyle}>Full Address</label>
                                    <Field icon={MapPin} error={errors.address}>
                                        <textarea value={data.address} onChange={e => setData('address', e.target.value)} style={{ ...fieldStyle, padding: '0.75rem 1rem 0.75rem 2.5rem', minHeight: '100px', resize: 'vertical' }} />
                                    </Field>
                                </div>
                            </div>
                        </SectionCard>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #f1f5f9', padding: '1.5rem', textAlign: 'center' }}>
                            <div style={{ width: '80px', height: '80px', borderRadius: '20px', background: '#f3f4f6', margin: '0 auto 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                                {employee.avatar ? <img src={`/storage/${employee.avatar}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <User size={32} color="#9ca3af" />}
                            </div>
                            <Link href={route('employees.show', employee.id)}>
                                <button style={{ width: '100%', background: '#fff', border: '1px solid #e5e7eb', color: '#111827', fontSize: '0.8rem', fontWeight: 800, padding: '8px', borderRadius: '8px', cursor: 'pointer' }}>View Profile</button>
                            </Link>
                        </div>
