...
import { ChevronLeft, Save, Upload, Palette, Layout, FileText, Image as ImageIcon,
         Building2, PenLine } from 'lucide-react';
...
export default function Edit({ auth, design, documentTypes }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        company_address:       design.company_address || '',
        company_tagline:       design.company_tagline || '',
        show_signature_lines:  Boolean(design.show_signature_lines),
        show_bank_details:     Boolean(design.show_bank_details),
        bank_details:          design.bank_details || '',
        header_logo:           null,
    ...
    return (
        <FigmaLayout user={auth.user}>
            <Head title={`Edit Template: ${design.name}`} />
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.75rem', paddingBottom: '4rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                    ...
                    <div>
                        <h1 style={{ fontSize: '1.75rem', fontWeight: 900, color: '#1e1b4b', margin: 0 }}>Edit Template</h1>
                        <p style={{ fontSize: '0.875rem', color: '#6b7280', fontWeight: 600, margin: '4px 0 0' }}>Modifying: <strong>{design.name}</strong> — {TYPE_META[design.type]?.label || design.type}</p>
                    </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: '2rem' }} className="rdm-grid">
                    ...
                    {/* ─── Form ─── */}
                    <form onSubmit={e => { e.preventDefault(); post(route('slip-designs.update', design.id)); }} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', flexWrap: 'wrap' }}>
                                    ...
                                    <div>
                                        ...
                                        <label htmlFor="logo_upload" style={{ display: 'inline-block', padding: '9px 14px', background: '#f8fafc', border: '1.5px solid #f0eeff', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 800, color: '#1e1b4b', cursor: 'pointer' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', flexWrap: 'wrap', paddingTop: '1.5rem', borderTop: '1px solid #f1f5f9' }}>
                                    ...
                                    <div>
                                        ...
                                        <label htmlFor="watermark_upload" style={{ display: 'inline-block', padding: '9px 14px', background: '#f8fafc', border: '1.5px solid #f0eeff', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 800, color: '#1e1b4b', cursor: 'pointer' }}>
                                            {watermarkPreview ? 'Replace Watermark' : 'Upload Watermark'}
                                        </label>
                        ...
                        <div style={card}>
                            ...
                            <div>
                                ...
                                <textarea value={data.footer_text} onChange={e => setData('footer_text', e.target.value)} style={{ ...inp, minHeight: '80px', resize: 'vertical' }} placeholder="Generated by system. All amounts in BDT." />
                            </div>
                        </div>
                        ...
                        <button type="submit" disabled={processing} style={{ height: '54px', background: 'linear-gradient(135deg, #1e1b4b, #4f46e5)', color: '#fff', border: 'none', borderRadius: '14px', fontSize: '1rem', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: '0 8px 25px rgba(30,27,75,0.2)', transition: 'all 0.2s' }}>
                            <Save size={20} /> {processing ? 'Saving...' : 'Update Template'}
                        </button>
                    </form>
