    ...
    return (
        <div className="min-h-screen bg-[#F3F4F6] flex font-sans text-[#111827]">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white transform transition-transform duration-300 ease-in-out border-r border-gray-200 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex-1 px-6 space-y-2">
                        <p className="px-4 text-[10px] uppercase font-bold text-gray-400 tracking-widest mb-4">Main Menu</p>
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`flex items-center space-x-4 px-4 py-3.5 rounded-2xl transition-all duration-200 group ${item.active
            ...
            <div className="flex-1 flex flex-col lg:ml-64 min-w-0">
                {/* Header */}
                <header className="sticky top-0 z-30 flex items-center justify-between h-20 px-8 bg-white/80 backdrop-blur-md border-b border-gray-100">
                    <div className="flex items-center flex-1 max-w-xl">
                        <button
                            onClick={() => setIsSidebarOpen(true)}
                            className="p-2 mr-4 text-gray-500 lg:hidden hover:bg-gray-100 rounded-xl transition-colors"
                        >
                    ...
                    <div className="flex items-center space-x-6">
                        <button className="relative p-2.5 text-gray-400 hover:text-gray-900 hover:bg-gray-50 rounded-2xl transition-all">
                        ...
                        <div className="flex items-center space-x-3 border-l border-gray-100 pl-6 cursor-pointer group">
                            ...
                            <div className="w-11 h-11 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl flex items-center justify-center text-gray-500 font-bold border-2 border-white shadow-sm overflow-hidden group-hover:border-[#22C55E]/20 transition-all">
